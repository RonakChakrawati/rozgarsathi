...
document.getElementById('search-form').addEventListener('submit', async function (e) {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      const jobsText = data.jobs.map((job, i) => `${i + 1}. ${job.title} (${job.salary})`).join('\n');

      const schemesText = data.schemes.map((scheme, i) => `${i + 1}. ${scheme.title}`).join('\n');
      const roadmapText = data.roadmap.map(step => `• ${step.step}`).join('\n');
...
~Generated by RozgarSathi`
      );
      console.log("WA TEXT =", waAllText);
